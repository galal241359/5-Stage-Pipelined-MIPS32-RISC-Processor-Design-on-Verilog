...
module pipe_MIPS32 (clk1, clk2);
 ...
 reg [31:0] Reg; // Register bank (32 x 32)
 ...
 always @(posedge clk1) // IF Stage
   if (HALTED == 0)
   begin
     if (((EX_MEM_IR == BEQZ) && (EX_MEM_cond == 1)) ||
         ((EX_MEM_IR == BNEQZ) && (EX_MEM_cond == 0)))
     begin
 ...
 always @(posedge clk2) // ID Stage
   if (HALTED == 0)
   begin
     if (IF_ID_IR == 5'b00000)
       ID_EX_A <= 0;
     else
       ID_EX_A <= #2 Reg[IF_ID_IR]; // "rs"
     if (IF_ID_IR == 5'b00000)
       ID_EX_B <= 0;
     else
       ID_EX_B <= #2 Reg[IF_ID_IR]; // "rt"
 ...
 begin
   if (TAKEN_BRANCH == 0)
   case (MEM_WB_type)
     RR_ALU: Reg[MEM_WB_IR] <= #2 MEM_WB_ALUOut;
     RM_ALU: Reg[MEM_WB_IR] <= #2 MEM_WB_ALUOut;
     LOAD:   Reg[MEM_WB_IR] <= #2 MEM_WB_LMD;
     HALT:   HALTED <= #2 1'b1;
   endcase
 end
endmodule
